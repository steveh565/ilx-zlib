when priority 10 RULE_INIT {
    ...
    #set unzipdata [ILX::call $handle "b64decode_and_unzip" $myData]
    if { [catch {ILX::call $handle "b64decode_and_unzip" $myData} result] } {
        log local0. "Whoops! ILX-ZLIB unexpected error calling method b64decode_and_unzip"
    }
    if { [lindex $result 0] } {
        set unzipdata [lindex $result 1]
        log local0. "Base64 Decoded and inflated: $unzipdata"
    } else {
        log local0. "Error: ILX-ZLIB initialization failed!"
    }
    ...
    #set zipdata [ILX::call $handle "deflate_and_b64encode" $xmldata]
    #log local0. "Deflated and Base64 Encoded: $zipdata"
}
